...
namespace WebsiteTN.Areas.Admin.Controllers
{
    ...
    public class AdminOrdersController : Controller
    {
        ...
        public AdminOrdersController(DataDbContext context, INotyfService notyfService)
        {
            _context = context;
        ...
        public async Task<IActionResult> ChangeStatus(int id, [Bind("OrderId,CustomerId,OrderDate,ShipDate,TransactStatusId,Deleted,Paid,PaymentDate,TotalMoney,PaymentId,Note,Address,LocationId,District,Ward")] Order order)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var orderVm = await _context.Orders.AsNoTracking()
                                                     .Include(x => x.Customer)
                                                     .FirstOrDefaultAsync(x => x.OrderId == id);
                    if (orderVm != null)
                    {
                        orderVm.Paid = order.Paid;
                        orderVm.Deleted = order.Deleted;
                        orderVm.TransactStatusId = order.TransactStatusId;
                        if (orderVm.Paid == true)
                        {
                            orderVm.PaymentDate = DateTime.Now;
                        }
                        if (orderVm.TransactStatusId == 5)
                        {
                            orderVm.Deleted = true;
                        }
                        if (orderVm.TransactStatusId == 3)
                        {
                            orderVm.ShipDate = DateTime.Now;
                        }
                    }
